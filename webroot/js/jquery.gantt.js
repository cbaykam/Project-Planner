		...
		gantt : new function(){
			...
			gnt.constract = function(c, o){
				...
				for(i=0;i<o.titles.length;i++){
					gnt_Header.append('<th class="GNT_head">Projects</th>');
				}
			...
			gnt.writeTask = function(c){
				...
				c.each(function(){
					...
					for(var i=0;i<p.tasks.length;i++){
						...
						var first = 1;
						var len = p.tasks[i]['events'].length;
						
						for(var j=0; j<len; j++){
							var s = p.tasks[i]['events'][j].start_date;
							var e = p.tasks[i]['events'][j].end_date;
							var barname = '.GNT_bar_' + j;
							if (!s || !e){
							...
							var barcode = '<div class="GNT_bar_'+j+'"><div class="GNT_bar_body_'+j+'">&nbsp;</div></div>';
							
							
							if(first == 1){
								$(".GNT_chart", trObj).append(barcode);
								first = 0;
								...
								var lastLeft = barWidth;							
							}else{
								$(".GNT_chart", trObj).append(barcode);
								var barLeft = gnt.diffDate(p.from.y, p.from.m, p.from.d, s.y, s.m, s.d)*gnt.graphSize;
								barLeft = barLeft - lastLeft;
								var barWidth = gnt.getRemainDate(s.y, s.m, s.d, e.y, e.m, e.d)*gnt.graphSize;
								lastLeft = lastLeft + barWidth;
							}
							if(j == len - 1){
								$(".GNT_chart", trObj).append('</div>');
							}
							var barbody = ".GNT_bar_body_" + j ;
							if (p.tasks[i]['events'][j].text){
								
								$(barbody, trObj).html(p.tasks[i]['events'][j].text);
							}
			...
			gnt.addTaskData = function(p, t){
				...
				for(var i=0;i<t.length;i++){
					...
					for(var j=0;j<t[i]['events'].length;j++){
						t[i]['events'][j].start_date = gnt.dateDiv(t[i]['events'][j].start_date);
						t[i]['events'][j].end_date = gnt.dateDiv(t[i]['events'][j].end_date);	
					}
					p.tasks.push(t[i]);
				}
				return p;
